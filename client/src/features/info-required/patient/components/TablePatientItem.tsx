...
export interface TablePatientItemProps {
  listOfPatient: Array<PatientResponse>;
  editRow?: (data: PatientResponse) => void;
  deleteRow?: (data: PatientResponse) => void;
  activateRow?: (data: PatientResponse) => void;
  assignRow?: (data: PatientResponse) => void;
  currentPage: number;
}

export const TablePatientItem: React.FC<TablePatientItemProps> = ({
  listOfPatient,
  editRow,
  deleteRow,
  activateRow,
  ...
  return (
    <>
      {listOfPatient.map((row, rowIndex) => (
        <tr key={row?.patientId} className="border-b border-neutral-100 text-center">
          ...
          <td className="p-3 w-[180px] flex gap-1.5 items-center justify-center">
            ...
            {!row.isActive && activateRow && (
              <Button
                title="Activar"
                id={`btnactivateRow-${row?.patientId}`}
                aria-data={row}
            ...
            {row.isActive && assignRow && (
              <Button
                title="Asignar como personal"
                id={`btnAssignRow-${row?.patientId}`}
                aria-data={row}
                classButton="transition-all duration-200 ease-in-out hover:text-primary-700"
                onClick={() => assignRow(row)}
              >
                <Icon.AssignStaff className="w-6 h-6" strokeWidth={1.2} />
              </Button>
            )}
          </td>
        </tr>
