...
export const SearchCertificateService = async ({
  ...
  try {
    ...
    // Respuesta exitosa, parseo el JSON y devuelvo el objeto DataResponseFromAPI<CertificateResponse>
    const data: DataResponseFromAPI<CertificateResponse> = await response.json();
    return data;
  } catch (error: unknown) {
