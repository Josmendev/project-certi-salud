...
export interface TableCertificateTypeItemProps {
  listOfCertificateTypes: Array<CertificateTypeResponse>;
  editRow?: (data: CertificateTypeResponse) => void;
  deleteRow?: (data: CertificateTypeResponse) => void;
  activateRow?: (data: CertificateTypeResponse) => void;
  currentPage: number;
}

export const TableCertificateTypeItem: React.FC<TableCertificateTypeItemProps> = ({
  listOfCertificateTypes,
  editRow,
  deleteRow,
  activateRow,
  currentPage,
  ...
  return (
    <>
      {listOfCertificateTypes.map((row, rowIndex) => (
        <tr key={row?.certificateTypeId} className="border-b border-neutral-100 text-center">
          ...
          <td className="p-3 w-[180px] flex gap-1.5 items-center justify-center">
            {editRow && (
              <Button
                title="Editar"
                id={`btnEditRow-${row?.certificateTypeId}`}
                aria-data={row}
            ...
            {row.isActive && deleteRow && (
              <Button
                title="Eliminar"
                id={`btnDeleteRow-${row?.certificateTypeId}`}
                aria-data={row}
            ...
            {!row.isActive && activateRow && (
              <Button
                title="Activar"
                id={`btnactivateRow-${row?.certificateTypeId}`}
                aria-data={row}
